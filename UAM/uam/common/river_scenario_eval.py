...
import uam.agents.continuous as agents
from uam.agents.base import _Agent
from uam.common.configparser import ConfigFile
from uam.common.open_scenario_eval import (COLREG_COLORS, NM_to_meter,
                                           dtr, get_ship_domain,
                                           head_inter, rtd)
from uam.configs import __path__ as cont_path
from uam.envs._envs.Plane import ED, bng_rel
                    ...
                    if f"TS{n}_N" in x.columns:
                        ...
                        if i == 2:
                            ...
                            for t in range(len(sim_t)):
                                ...
                                # relative bearing
                                bng_rel_TS = bng_rel(N0=x["OS_N"][t], E0=x["OS_E"][t], N1=x[f"TS{n}_N"][t],
                                                     E1=x[f"TS{n}_E"][t], head0=x["OS_head"][t])

                                # ship domain
                                D = get_ship_domain(A=SD_A, B=SD_B, C=SD_C, D=SD_D, OS=None, TS=None, ang=bng_rel_TS)

                                # dist
                                d = ED(N0=x["OS_N"][t], E0=x["OS_E"][t], N1=x[f"TS{n}_N"][t], E1=x[f"TS{n}_E"][t],
                                       sqrt=True) - D
                                dists.append(d)
                        ...
                        # ----------- rule violations -----------
                            ...
                            rule_vios = []
                            for t in range(len(sim_t)):
                                bin = violates_river_traffic_rules(N0=x["OS_N"][t], E0=x["OS_E"][t],
                                                                   head0=x["OS_head"][t], v0=x["OS_V"][t],
                                                                   N1=x[f"TS{n}_N"][t], E1=x[f"TS{n}_E"][t],
                                                                   head1=x[f"TS{n}_head"][t], v1=x[f"TS{n}_V"][t])
