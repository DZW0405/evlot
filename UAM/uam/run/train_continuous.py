...
from uam.agents import continuous as agents
from uam.agents.base import _Agent
from uam.common.configparser import ConfigFile
from uam.common.logging_func import EpochLogger
from uam.common.logging_plot import plot_from_progress
from uam.wrappers import get_wrapper
...
def train(c: ConfigFile, agent_name: str):
    ...
    # 保存环境文件以实现可追溯性
    try:
        entry_point = vars(gym.envs.registry[c.Env.name])["entry_point"][9:]
        shutil.copy2(src="uam/envs/_envs/" + entry_point + ".py", dst=agent.logger.output_dir)
    except:
        logger.warning(
...
def save_weights(agent: _Agent, eval_ret) -> None:
    ...
    # 在多智能体问题中不存在最佳权重保存，因为最佳权重的定义不再那么直接明确了
    if agent.is_multi:
        best_weights = False
